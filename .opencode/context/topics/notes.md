...
- Toolbar = the **full standard formatting set** (bold/italic/strikethrough, headings 1-3, quote, lists, code, table, hr, link, image, preview, side-by-side, fullscreen, guide) **plus** the three custom buttons: `add-task` (promote selection to task), `cleanify` (tidy whole note via LLM), `undo-cleanify` (single-step restore). (The original design shipped custom-buttons-only to keep formatting as Cleanify's job; user feedback 2026-07 asked for the full toolbar back.) The custom actions are also surfaced as standalone HTML buttons (`#notePromoteBtn`, `#cleanifyBtn`, `#undoCleanifyBtn`) below the editor.
