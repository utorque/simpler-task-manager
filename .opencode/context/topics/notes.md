...
- Reuses the existing `GET /api/spaces` for its Space switcher; filter state (selected Space) persisted to `localStorage`. Chip clicks: plain = single space, Ctrl+click = toggle the space in/out of the multi-space filter, Alt+click = exclude the space (greyed `.excluded` chip, its notes hidden until Alt+clicked again; persisted; "All" resets filter + exclusions) — same handler contract as the board's chips in `app.js`. The space filter chips live in a **secondary header** (`.notes-toolbar`, same layout as the tasks toolbar) above the notes layout, with a slim **Download** button on its right: exports the Ctrl+click-selected notes (or, with no selection, every note in view) as one client-side markdown file (`notes-YYYY-MM-DD.md`, notes joined by `---`, `# Title` heading skipped when a note body already starts with one). Ctrl+click a note row toggles it in/out of the download selection (`.note-row.selected`); plain click still opens the note; Esc clears; the selection is pruned to visible notes on every list reload.
...
- **Mandatory config:** `autosave: {enabled: false}` (EasyMDE's built-in localStorage autosave would shadow the backend `PUT` path), `status: false`, `sideBySideFullscreen: false` (side-by-side must stay inside the notes layout — EasyMDE defaults to taking over the whole screen).
- **Preview-by-default (2026-07):** opening an existing note enters EasyMDE preview mode (rendered markdown); double-clicking anywhere on the preview (`.editor-preview-full`, links excepted) switches to edit mode and focuses CodeMirror. New/empty notes open straight in edit mode. Because the preview pane only re-renders on toggle, `openNote`/Cleanify/Undo write the buffer with preview off and flip it back (`setPreviewMode` / `replaceEditorContent` in `notes.js`).
...
- `POST /api/notes/<id>/cleanify` → body `{}` → `{content: <string>}`. Builds `Config.NOTES_CLEANIFY_PROMPT` + the note's Space context suffix + the note's date (`created_at`, `YYYY-MM-DD` — the prompt instructs the AI to put it in italics directly below the title), calls `cleanify_note_with_ai(note.content_markdown, system_prompt)`. The prompt (`src/prompts/notes_cleanify.md`) enforces a structured output — one `# Title`, the date line, `##` subtitles when multi-topic, bold key points, `-` bullets — while keeping the original preserve-meaning/no-translation rules. **Does not persist** (the client replaces editor content + the existing debounced `PUT` autosave persists). On AI failure → `{content: <original note content>}` (graceful degradation via `cleanify_note_with_ai`'s try/except).
