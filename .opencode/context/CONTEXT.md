...
- `src/routes/` — `pages` (/, /notes→/#notes, login), `tasks`, `spaces`, `notes`, `mailboxes`, `calendar_sources` (+ external-events), `schedule` (+ logs), `hermes_proxy` (/hermes-ui/* same-origin proxy).
...
- `tests/` — pytest harness (in-memory SQLite, `StubAIProvider`, patched IMAP seam) + route-layer integration tests + pure-data scheduler suite + MCP tool suite + hermes-proxy suite. 159 tests.
...
- **Hermes destination** (optional, PRD 002): 6th tab (`6`, `#hermes`) lazily iframing hermes-webui. Default path: the `hermes-webui` compose service (chat UI + agent auto-installed into `./hermes-home`, no host port) proxied SAME-ORIGIN at `/hermes-ui/` by `routes/hermes_proxy.py` (login-gated streaming proxy; strips `X-Frame-Options`/`frame-ancestors`, rewrites redirects under the prefix) — zero reverse-proxy changes; enabled by `HERMES_WEBUI_INTERNAL_URL`. Alternative: `HERMES_WEBUI_URL` iframes an externally served webui (sibling subdomain + frame-ancestors). Both unset ⇒ tab, view, and help-modal row all absent (template-conditional). The agent acts on the app through the MCP sidecar, never the iframe. Setup: `doc/setup-hermes-integration.md`.
...
PRD `002` (Hermes Agent integration) **phases 1–2 implemented, fully containerized** (2026-07-05): `API_TOKEN` bearer auth + `actor='agent'` audit attribution, the `mcp_server/` FastMCP sidecar (~26 tools, compose service `mcp`, compose-network-only), the `hermes-webui` compose service (chat UI + agent in `./hermes-home`), and the `#hermes` destination embedded via the same-origin `/hermes-ui/` proxy. Setup walkthrough: `doc/setup-hermes-integration.md`. Remaining per the PRD: Phase 3 automations (Hermes-side cron: morning brief, inbox sweep — needs the Hermes gateway process, not in compose yet) and Phase 4 options (native chat pane over Hermes's OpenAI-compatible API, `delete_task` unlock behind approvals, preference learning).
