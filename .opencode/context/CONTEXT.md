...
- `tests/` — pytest harness (in-memory SQLite, `StubAIProvider`, patched IMAP seam) + route-layer integration tests + pure-data scheduler suite + MCP tool suite. 149 tests.
...
- **Hermes destination** (optional, PRD 002): 6th tab (`6`, `#hermes`) lazily iframing a self-hosted hermes-webui (`HERMES_WEBUI_URL` env; unset ⇒ tab, view, and help-modal row all absent — template-conditional). Needs a reverse proxy on the webui side allowing `frame-ancestors` for the app origin. The agent behind it acts on the app through the MCP sidecar, never the iframe.
...
PRD `002` (Hermes Agent integration) **phases 1–2 implemented** (2026-07-05): `API_TOKEN` bearer auth + `actor='agent'` audit attribution, the `mcp_server/` FastMCP sidecar (~26 tools, compose service `mcp` on loopback :8765), and the optional `#hermes` iframe destination (`HERMES_WEBUI_URL`). Remaining per the PRD: Phase 3 automations (Hermes-side cron: morning brief, inbox sweep — needs a live Hermes to configure) and Phase 4 options (native chat pane over Hermes's OpenAI-compatible API, `delete_task` unlock behind approvals, preference learning). Deployment/reverse-proxy recipe: PRD §4.2 + `mcp_server/README.md`.
