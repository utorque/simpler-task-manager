...
**Layout model — locked viewport, one scroller.** Phones are height-locked exactly like desktop: `html, body { height: 100dvh; overflow: hidden }`, and the height budget is

```
app-main = 100dvh − --ph-header-h (50px) − --ph-nav-h (54px + safe-area)
```

Each destination then *divides* that budget with flexbox — a fixed toolbar row plus exactly one child that scrolls itself — instead of growing the page. `viewDisplayMode()` in `app.js` is the JS half: on phones the Tasks, Notes and Calendar views are set to `display:flex` (they stay `block` on desktop, where only the Assistant view is a flexbox). This is what keeps **double scrollbars** out of the kanban, the note editor and the assistant iframe: nothing nests a scroller inside another scroller, and no view scrolls the page.

- **Header**: **one row** — brand, the quick-capture input inline (it takes whatever width is left), action icons at the right. The tab nav is lifted out to a fixed **bottom tab bar** (icons + tiny labels, `env(safe-area-inset-bottom)`-padded), so it costs the header no width and `app-main` already subtracts its height.
- **Toolbars**: one row everywhere — the space dropdown takes the slack and the actions become **icon-only**. Every label the phone layer drops is wrapped in a `.btn-label` span in `index.html` (Board / Overview / Download / Share / Stop sharing / Files / Settings), so hiding it is one CSS rule and the desktop markup renders identically.
- **Space filter**: a chip row for ~10 spaces would eat most of a phone screen, so every `.space-chips` container (board, Notes, Assistant) gets a companion `<select>` built by `syncMobileSpaceSelect()` — the chips stay in the DOM, just hidden. Deliberately **single-select**: Ctrl+click multi-space and Alt+click exclude stay desktop-only, and picking a space from the dropdown clears exclusions. A filter set up on desktop that the dropdown can't express (several spaces, or exclusions) shows a disabled "Multiple spaces" entry rather than misreporting it.
- **Tasks**: the 4-column board becomes full-width **horizontal scroll-snap columns** (swipe between them, a sliver of the next peeks) that fill the height budget; the column scrolls, the page never does. Drag is disabled on touch (`initBoardSortables`/`initSortable` skipped when `isMobile()`), so **tapping a card opens the action sheet** (`openCardActionSheet` — status · freeze · Edit…; a plain DOM bottom sheet, not a Bootstrap modal) rather than jumping straight to the edit modal. Hover-only card buttons are pinned visible.
- **Overview**: the single scrolling child of the Tasks view; stat cards go 2×2 and the **"Show done"** bar + its list are ordered **last** (CSS `order`, so the desktop order is untouched).
...
- **Note editor**: the overlay is height-locked to the viewport and passes a definite height down the chain (overlay → `.EasyMDEContainer` → `.CodeMirror`), so the note is always exactly as tall as the space available and only the editor (or its rendered preview) scrolls. `NotesView.refresh()` re-measures CodeMirror when the overlay opens or the breakpoint flips.
- **Calendar**: `initCalendar` picks a `listWeek` view + slimmed toolbar on phones; the task sidebar and the calendar become two stacked panes sharing the budget (each scrolls itself), so the calendar is always on screen.
- **Assistant**: the iframe gets an **exact pixel height** (`app-main` − toolbar), which is the "necessary info" the embedded Chainlit UI needs — its own full-height layout then resolves against the space we gave it and it scrolls its own message list. The workspace **Files** button and drawer are off (a file tree needs a pointer and width). Phone rules *inside* the iframe live in `chat/public/simpler.css`, gated on `html.simpler-phone`, which `simpler-bridge.js` mirrors from the shell's `body.is-mobile` — deliberately not from the iframe's own width, which a narrowed desktop window can also make small. They hide the **starters** (they push the composer around on a phone; the backend still ships them, so a wider viewport gets them back with no reload), wrap the composer's control row, and sit the empty-state composer at the bottom.
- **Modals**: render as full-screen sheets.

Verification: a Playwright pass (local harness) asserts, at 320/360/393/430/640×short, on every destination: no horizontal overflow, **no page scroll**, and a note-editor overlay exactly the viewport height with its action row inside it; it also drives the real flows (tap-card → sheet → status change, space dropdown, a chat round trip in the iframe, a phone↔desktop breakpoint round trip). Desktop is checked by **pixel-diffing** screenshots of every destination at 641/700/768/900/1024/1440 before vs after — the only permitted deltas are the notes list's relative timestamps and sub-threshold antialiasing.
...
- ✅ Header down to **one row** (brand · inline quick capture · action icons); toolbars to one row with **icon-only** actions (labels wrapped in `.btn-label` spans so desktop markup is unchanged)
- ✅ Kanban: the per-card `…` button is gone — **tapping a card opens the action sheet** (status · freeze · Edit…). Board/Overview sit inline with the space dropdown as icons; the Overview's "Show done" bar moves to the bottom (CSS `order`) and stat cards go 2×2
- ✅ Note editor: overlay height-locked to the viewport with a definite-height chain down to CodeMirror, so the note is exactly as tall as the space available and only the editor/preview scrolls (`NotesView.refresh()` re-measures on open and on breakpoint flips)
- ✅ Assistant: iframe gets an exact pixel height; the shell tells the embedded UI it is on a phone via `html.simpler-phone` (mirrored from `body.is-mobile` by `simpler-bridge.js`, not guessed from iframe width), and `chat/public/simpler.css` uses it to hide the **starters**, wrap the composer controls and bottom-align the empty state. Workspace **Files** button + drawer off on phones
- ✅ Calendar: task sidebar and calendar become two stacked panes each scrolling itself, so the calendar is always on screen
- ✅ Desktop non-regression by **pixel diff** at 641/700/768/900/1024/1440 (only deltas: relative timestamps + sub-threshold antialiasing); 363 backend tests green
